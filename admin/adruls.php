...
<?php if ($_GET['catid']=='viewrul') { ?>
<div class='title'> <em>View All Consultant News</em> </div>

<?php } if($_GET['catid']=='editrul') {?>

<div class='title'> <em>Edit The Consultant News</em> </div>
...
$query = "INSERT INTO newstype (`title`,`category`,`description`,`images`,`slideshow`,`author`,`nstatus`,`ctime`,`cdate`,`btime`,`bdate`) Values ('".$_POST['sportstitle']."','".sports."','".$_POST['editor1']."','".$filePath."','".$_POST['slide']."','".$_POST['reporter']."','". 1 ."','".$timestamp."','".$cdate."','".$presenttime."','".$convertedDATE."')";
...
<script type="text/javascript">
    ...
    <?php
        $query= "SELECT * FROM self WHERE category='adrules' order by sfid DESC";
        $rsquery= mysql_query($query); 
        while ($vquery=mysql_fetch_array($rsquery))
       {
     ?>
    <tr> 
    <td width="409" align="center" style="font-family:SolaimanLipi">Ad Rules </td>
    </td>
    <td width="56" align="left">
	<a href="admin.php?action=adrules&catid=editrul&sfid=<?php echo $vquery['sfid'];?>"><i class="fa fa-edit" title="News Edit"></i></a>&nbsp;&nbsp;
    </td>
    </tr>
    <?php 
       }
...
<div class="page"> 
 ...
 <?php
  $id= $_GET["sfid"]; 
  $query= "SELECT * FROM self WHERE category='adrules' AND sfid='".$_GET["sfid"]."'";
  $rsquery= mysql_query($query); 
  $vquery= mysql_fetch_array($rsquery);
?>


<form action="admin.php?action=editnews&catid=31" method="post" enctype="multipart/form-data">
