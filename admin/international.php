...
<?php if ($_GET['catid']=='newinternational') { ?>
<div class='title'> <em>Add New Inernationl News</em> </div>
...
<?php if ($_GET['catid']=='viewinternational') { ?>
<div class='title'> <em>View All Inernationl News</em> </div>
<br />
<?php } if($_GET['catid']=='editinternationl') {?>

<div class='title'> <em>Edit The Inernationl News</em> </div>
<?php } ?>
...
<?php 
 ...
 // $description = htmlentities(addslashes(stripslashes($_POST['editor1'])), ENT_QUOTES | ENT_IGNORE, "UTF-8");
      //$description =  $_POST['editor1'];
      //$inpute=preg_replace("/<div>(.*?)<\/div>/", "$1", $description);
...
if($_POST['intertitle']!='' && $_FILES['up_images']!='' && $_POST['reporter']!='' && $_POST['editor1']!='')
 {
...
$query = "INSERT INTO newstype (`title`,`category`,`description`,`images`,`slideshow`,`author`,`nstatus`,`bracking`,`youtube`,`important`,`ctime`,`cdate`,`btime`,`bdate`) 
Values ('".$_POST['intertitle']."','".international."','".$content."','".$filePath."','". $_POST['slide']."','".$_POST['reporter']."','". 1 ."','".$_POST['bknews'] ."','".$_POST['youtube']."'
,'".$_POST['important']."','".$CUR_D_Bangladesh."','".$GM_D."','".$presenttime."','".$presentDate."')";
...
if($Query2)
	...
	{
$msg= "International News Insert Successfully";
header("location:admin.php?action=international&catid=viewinternational&msg=$msg"); //&msg=$msg
	
	}else{ 
	
$error= "Some Information Was Wrong";
header("location:admin.php?action=international&catid=newinternational&error=$error"); //&error=$error
	  }
	  
	}
	else{
$error= "Some Information Was Wrong";	
header("location:admin.php?action=international&catid=newinternational&error=$erro"); //&error=$error
	}
   }
?>
...
<?php 
 if($_GET['catid']=='newinternational')
  {
?>
<div class="page"> 

<form action="admin.php?action=international&catid=1" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">International News Head line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="intertitle" placeholder="International News Head line"></div>
...
<?php } ?>
 ...
 <?php 
 if($_GET['catid']=='viewinternational')
  {
?>
...
if($id!="")
{
    $query2= "select * FROM newstype WHERE category='international' AND nid='$id'";
    $rsquery2 =mysql_query($query2);
    $vquery2 = mysql_fetch_assoc($rsquery2);
    unlink($vquery2['images']);
	
	$qryDelete="DELETE FROM newstype WHERE category='international' AND nid= '$id'";
	$rsDelete=mysql_query($qryDelete);
	
	if($rsDelete)
	{
		$demsg= "Delete Successfull";
		header("Location:admin.php?action=international&catid=viewinternational&demsg=$demsg");
		die();
	}
	else {
	    $delerror= "Delete Failed";
		header("Location:admin.php?action=international&catid=viewinternational&delerror=$delerror");
		die();
	}	
}
...
<a href="admin.php?action=international&catid=editinternationl&nid=<?php echo $vquery['nid'];?>"><i class="fa fa-edit" title="News Edit"></i></a>&nbsp;&nbsp;
<a href="admin.php?action=international&catid=viewinternational&nid=<?php echo $vquery['nid'];?>" onclick="return confirmasubmit()"><i class="fa fa-trash-o" title="Delete News"></i></a>
</td>
</tr>
<?php 
   }
...
<div class="paigination"><span class="paiglink"><?php pagination($page,$nrow,"admin.php?action=finance&catid=viewfinance&", $perpa);?></span></div>
<div class="clr">  </div>
</div>
<?php
  }
...
<div class="page"> 
 ...
 <?php
  $id= $_GET["nid"]; 
  $query= "SELECT * FROM newstype WHERE category='international' AND nid='".$_GET["nid"]."'";
  $rsquery= mysql_query($query); 
...
<form action="admin.php?action=editnews&catid=4" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">International News Head line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="intertitle" placeholder="International News Head line" 
