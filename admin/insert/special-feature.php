<?php if ($_GET['catid']=='newspecial') { ?>
<div class='title'> <em>Special Feature News</em> </div>
...
<?php if ($_GET['catid']=='viewspecial') { ?>
<div class='title'> <em>View All Special Feature News</em> </div>

<?php } if($_GET['catid']=='editspecial') {?>

<div class='title'> <em>Edit The Special Feature News</em> </div>
<?php } ?>
...
if($_POST['sptitle']!='' && $_FILES['up_images']!='' && $_POST['reporter']!='')
 {
...
$query = "INSERT INTO newstype (`title`,`category`,`description`,`images`,`slideshow`,`author`,`nstatus`,`bracking`,`youtube`,`ctime`,`cdate`,`btime`,`bdate`) Values ('".$_POST['sptitle']."','".specialfeature."','".$content."','".$filePath."','".$_POST['slide']."','".$_POST['reporter']."','". 1 ."',
...
header("location:admin.php?action=special&catid=newspecial&error=$error"); //&error=$error
	  }
...
<form action="admin.php?action=special&catid=1" method="post" enctype="multipart/form-data">
...
if($id!="")
{
    $query2= "select * FROM newstype WHERE category='specialfeature' AND nid='$id'";
    $rsquery2 =mysql_query($query2);
    $vquery2 = mysql_fetch_assoc($rsquery2);
    unlink($vquery2['images']);
	
	$qryDelete="DELETE FROM newstype WHERE category='specialfeature' AND nid= '$id'";
	$rsDelete=mysql_query($qryDelete);
	
	if($rsDelete)
	{
		$demsg= "Delete Successfull";
		header("Location:admin.php?action=special&catid=viewspecial&1&demsg=$demsg");
		die();
	}
	else {
	    $delerror= "Delete Failed";
		header("Location:admin.php?action=special&catid=viewspecial&delerror=$delerror");
		die();
	}	
}
...
<td width="409" align="center">Special Feature News Title</td>
...
<a href="admin.php?action=special&catid=editspecial&nid=<?php echo $vquery['nid'];?>"><i class="fa fa-edit" title="News Edit"></i></a>&nbsp;&nbsp;
<a href="admin.php?action=special&catid=viewspecial&nid=<?php echo $vquery['nid'];?>" onclick="return confirmasubmit()"><i class="fa fa-trash-o" title="Delete News"></i></a>
...
<div class="paigination"><span class="paiglink"><?php pagination($page,$nrow,"admin.php?action=special&catid=viewspecial&", $perpa);?></span></div> 
<div class="clr">  </div>
</div>
<?php
  }
...
<?php if($_GET['catid']=='editspecial') {?>
 ...
 <?php
  $id= $_GET["nid"]; 
  $query= "SELECT * FROM newstype WHERE category='specialfeature' AND nid='".$_GET["nid"]."'";
  $rsquery= mysql_query($query); 
...
<form action="admin.php?action=editnews&catid=12" method="post" enctype="multipart/form-data">
