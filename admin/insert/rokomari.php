<?php if ($_GET['catid']=='newrokomari') { ?>
<div class='title'> <em>Rokomari News</em> </div>
...
<?php if ($_GET['catid']=='viewrokomari') { ?>
<div class='title'> <em>View All Rokomari News</em> </div>

<?php } if($_GET['catid']=='editrokomari') {?>

<div class='title'> <em>Edit The Rokomari News</em> </div>
<?php } ?>
...
if($_POST['rokotitle']!='' && $_FILES['up_images']!='' && $_POST['reporter']!='')
 {
...
$query = "INSERT INTO newstype (`title`,`category`,`description`,`images`,`slideshow`,`author`,`nstatus`,`bracking`,`youtube`,`ctime`,`cdate`,`btime`,`bdate`) Values ('".$_POST['rokotitle']."','".rokomary."','".$content."','".$filePath."','". $_POST['slide']."','".$_POST['reporter']."','". 1 ."','".$_POST['bknews'] ."'
...
if($Query2)
	...
	{
$msg= "Rokomay News Insert Successfully";
header("location:admin.php?action=rokomari&catid=viewrokomari&msg=$msg"); //&msg=$msg
	
	}else{ 
	
$error= "Some Information Was Wrong";
header("location:admin.php?action=rokomari&catid=newrokomari&error=$error"); //&error=$error
	  }
...
<form action="admin.php?action=rokomari&catid=1" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">Rokomary News Head line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="rokotitle" placeholder="Rokomary News Head line"></div>
...
if($id!="")
{
    $query2= "select * FROM newstype WHERE category='rokomary' AND nid='$id'"; 
    $rsquery2 =mysql_query($query2);
    $vquery2 = mysql_fetch_assoc($rsquery2);
    unlink($vquery2['images']);
	
	$qryDelete="DELETE FROM newstype WHERE category='rokomary' AND nid='$id'";
	$rsDelete=mysql_query($qryDelete);
	
	if($rsDelete)
	{
		$demsg= "Delete Successfull";
		header("Location:admin.php?action=rokomari&catid=viewrokomari&1&demsg=$demsg");
		die();
	}
	else {
	    $delerror= "Delete Failed";
		header("Location:admin.php?action=rokomari&catid=viewrokomari&delerror=$delerror");
		die();
	}	
}
...
<td width="409" align="center">Rokomary News Title</td>
...
<?php
	  $i=1;
 $perpa=20;
 $re1=mysql_query("select count(nid) row FROM newstype WHERE category='rokomary' order by nid");
...
<a href="admin.php?action=rokomari&catid=editrokomari&nid=<?php echo $vquery['nid'];?>"><i class="fa fa-edit" title="News Edit"></i></a>&nbsp;&nbsp;
<a href="admin.php?action=rokomari&catid=viewrokomari&nid=<?php echo $vquery['nid'];?>" onclick="return confirmasubmit()"><i class="fa fa-trash-o" title="Delete News"></i></a>
...
<div class="paigination"><span class="paiglink"><?php pagination($page,$nrow,"admin.php?action=rokomari&catid=viewrokomari&", $perpa);?></span></div>
<div class="clr"></div>
</div>
<?php
  }
 ...
 <?php
  $id= @$_GET["nid"]; 
  $query= "SELECT * FROM newstype WHERE category='rokomary' AND nid='".$_GET["nid"]."'";
  $rsquery= mysql_query($query); 
  $vquery= mysql_fetch_array($rsquery);
?>
 
 
 <form action="admin.php?action=editnews&catid=20" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">Rokomary News Head line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="rokotitle" placeholder="Rokomary News Head line"
