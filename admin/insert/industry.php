<?php if ($_GET['catid']=='newindustry') { ?>
<div class='title'> <em>Insert your Art - Literature News</em> </div>
...
<?php if ($_GET['catid']=='viewindustry') { ?>
<div class='title'> <em>View All Art - Literature News</em> </div>

<?php } if($_GET['catid']=='editindustry') {?>

<div class='title'><em>Edit The Art - Literature  News</em> </div>
<?php } ?>
...
$query = "INSERT INTO newstype (`title`,`category`,`description`,`images`,`slideshow`,`author`,`nstatus`,`bracking`,`youtube`,`ctime`,`cdate`,`btime`,`bdate`) Values ('".$_POST['Intitle']."','".artculture."','".$content."','".$filePath."','". $_POST['slide']."','".$_POST['reporter']."','". 1 ."','".$_POST['bknews'] ."'
...
if($Query2)
	...
	{
$msg= "Art - Literature News Insert Successfully";
header("location:admin.php?action=industry&catid=viewindustry&msg=$msg"); //&msg=$msg
	
	}else{ 
	
$error= "Some Information Was Wrong";
header("location:admin.php?action=industry&catid=newindustry&error=$error"); //&error=$error
	  }
...
<form action="admin.php?action=industry&catid=1" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">Art - Literature Head line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="Intitle" placeholder="Art - Literature News Head line"></div>
...
if($id!="")
{
    $query2= "select * FROM newstype WHERE category='artculture' AND nid='$id'"; 
    $rsquery2 =mysql_query($query2);
    $vquery2 = mysql_fetch_assoc($rsquery2);
    unlink($vquery2['images']);
	
	$qryDelete="DELETE FROM newstype WHERE category='artculture' AND nid = '$id'";
	$rsDelete=mysql_query($qryDelete);
	
	if($rsDelete)
	{
		$demsg= "Delete Successfull";
		header("Location:admin.php?action=industry&catid=viewindustry&demsg=$demsg");
		die();
	}
	else {
	    $delerror= "Delete Failed";
		header("Location:admin.php?action=industry&catid=viewindustry&delerror=$delerror");
		die();
	}	
}
...
<td width="409" align="center">Art - Literature News Title</td>
...
<a href="admin.php?action=industry&catid=editindustry&nid=<?php echo $vquery['nid'];?>"><i class="fa fa-edit" title="News Edit"></i></a>&nbsp;&nbsp;
<a href="admin.php?action=industry&catid=viewindustry&nid=<?php echo $vquery['nid'];?>" onclick="return confirmasubmit()"><i class="fa fa-trash-o" title="Delete News"></i></a>
...
<div class="paigination"> <span class="paiglink"><?php pagination($page,$nrow,"admin.php?action=industry&catid=viewindustry&", $perpa);?></span> </div>
<div class="clr"></div>
</div>
...
<?php
  $id= @$_GET["nid"]; 
  $query= "SELECT * FROM newstype WHERE category='artculture' AND nid='".$_GET["nid"]."'";
  $rsquery= mysql_query($query); 
  $vquery= mysql_fetch_array($rsquery);
?>


<form action="admin.php?action=editnews&catid=18" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">Art - Literature Head line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="Intitle" placeholder="Art - Literature News Head line"
