<?php if ($_GET['catid']=='agnew') { ?>
<div class='title'> <em>Agreculture News</em> </div>
...
<?php if ($_GET['catid']=='viewag') { ?>
<div class='title'> <em>View Agreculture News</em> </div>

<?php } if($_GET['catid']=='editag') {?>

<div class='title'> <em>Edit Agreculture News</em> </div>
<?php } ?>
...
if($_POST['agtitle']!='' && $_FILES['up_images']!='' && $_POST['reporter']!='')
 {
...
$query = "INSERT INTO newstype (`title`,`category`,`description`,`images`,`slideshow`,`author`,`nstatus`,`bracking`,`youtube`,`ctime`,`cdate`,`btime`,`bdate`) Values ('".$_POST['agtitle']."','".krishienvironment."','".$content."','".$filePath."','".$_POST['slide']."','".$_POST['reporter']."','". 1 ."','".$_POST['bknews']."','".$_POST['youtube']."','".$CUR_D_Bangladesh."','".$GM_D."','".$presenttime."','".$presentDate."')";
...
if($Query2)
	...
	{
$msg= "Agriculture  News Insert Successfully";
header("location:admin.php?action=agenvirinment&catid=viewag&msg=$msg"); //&msg=$msg
	
	}else{ 
	
$error= "Some Information Was Wrong";
header("location:admin.php?action=agenvirinment&catid=agnew&error=$error"); //&error=$error
	  }
...
<form action="admin.php?action=agenvirinment&catid=1" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">Agreculture News Head line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="agtitle" placeholder="Agreculture News Head line"></div>
...
if($id!="")
{
    $query2= "select * FROM newstype WHERE category='krishienvironment' AND nid='$id'"; 
    $rsquery2 =mysql_query($query2);
    $vquery2 = mysql_fetch_assoc($rsquery2);
    unlink($vquery2['images']);
	
	$qryDelete="DELETE FROM newstype WHERE category='krishienvironment' AND nid = '$id'";
	$rsDelete=mysql_query($qryDelete);
	
	if($rsDelete)
	{
		$demsg= "Delete Successfull";
		header("Location:admin.php?action=agenvirinment&catid=viewag&1&demsg=$demsg");
		die();
	}
	else {
	    $delerror= "Delete Failed";
		header("Location:admin.php?action=agenvirinment&catid=viewag&delerror=$delerror");
		die();
	}	
}
...
<td width="409" align="center">Agreculture News Title</td>
...
<a href="admin.php?action=agenvirinment&catid=editag&nid=<?php echo $vquery['nid'];?>"><i class="fa fa-edit" title="News Edit"></i></a>&nbsp;&nbsp;
<a href="admin.php?action=agenvirinment&catid=viewag&nid=<?php echo $vquery['nid'];?>" onclick="return confirmasubmit()"><i class="fa fa-trash-o" title="Delete News"></i></a>
...
<div class="paigination"><span class="paiglink"><?php pagination($page,$nrow,"admin.php?action=agenvirinment&catid=viewag&", $perpa);?></span></div>
<div class="clr"></div>
</div>
...
<?php
 ...
 <?php
  $id= $_GET["nid"]; 
  $query= "SELECT * FROM newstype WHERE category='krishienvironment' AND nid='".$_GET["nid"]."'";
  $rsquery= mysql_query($query); 
...
?>
 ...
 <form action="admin.php?action=editnews&catid=14" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">Agreculture News Head line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="agtitle" placeholder="Agreculture News Head line"
