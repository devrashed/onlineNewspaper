<?php if ($_GET['catid']=='newlifestyle') { ?>
<div class='title'> <em>Life style News</em> </div>
...
<?php if ($_GET['catid']=='viewlifestyle') { ?>
<div class='title'> <em>View All Life style News</em> </div>

<?php } if($_GET['catid']=='editlifestyle') {?>

<div class='title'> <em>Edit The Life style News</em> </div>
<?php } ?>
...
$query = "INSERT INTO newstype (`title`,`category`,`description`,`images`,`slideshow`,`author`,`nstatus`,`bracking`,`youtube`,`ctime`,`cdate`,`btime`,`bdate`) Values ('".$_POST['Intitle']."','".lifestyle."','".$content."','".$filePath."','". $_POST['slide']."','".$_POST['reporter']."','". 1 ."','".$_POST['bknews'] ."'
...
if($Query2)
	...
	{
$msg= "Life style  News Insert Successfully";
header("location:admin.php?action=lifestyle&catid=viewlifestyle&msg=$msg"); //&msg=$msg
	
	}else{ 
	
$error= "Some Information Was Wrong";
header("location:admin.php?action=lifestyle&catid=newlifestyle&error=$error"); //&error=$error
	  }
...
<?php 
 if($_GET['catid']=='newlifestyle')
  {
?>

<div class="page">

<form action="admin.php?action=lifestyle&catid=1" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">Life Style Head line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="Intitle" placeholder="Life Style News Head line"></div>
...
if($id!="")
{
    $query2= "select * FROM newstype WHERE category='lifestyle' AND nid='$id'"; 
    $rsquery2 =mysql_query($query2);
    $vquery2 = mysql_fetch_assoc($rsquery2);
    unlink($vquery2['images']);
	
	$qryDelete="DELETE FROM newstype WHERE category='lifestyle' AND nid = '$id'";
	$rsDelete=mysql_query($qryDelete);
	
	if($rsDelete)
	{
		$demsg= "Delete Successfull";
		header("Location:admin.php?action=lifestyle&catid=editlifestyle&demsg=$demsg");
		die();
	}
	else {
	    $delerror= "Delete Failed";
		header("Location:admin.php?action=lifestyle&catid=viewlifestyle&delerror=$delerror");
		die();
	}	
}
...
<td width="409" align="center">Life Style News Title</td>
...
<a href="admin.php?action=lifestyle&catid=editlifestyle&nid=<?php echo $vquery['nid'];?>"><i class="fa fa-edit" title="News Edit"></i></a>&nbsp;&nbsp;
<a href="admin.php?action=lifestyle&catid=viewlifestyle&nid=<?php echo $vquery['nid'];?>" onclick="return confirmasubmit()"><i class="fa fa-trash-o" title="Delete News"></i></a>
...
<div class="page">
 ...
 <?php
  $id= @$_GET["nid"]; 
  $query= "SELECT * FROM newstype WHERE category='lifestyle' AND nid='".$_GET["nid"]."'";
  $rsquery= mysql_query($query); 
  $vquery= mysql_fetch_array($rsquery);
?>

 
<form action="admin.php?action=editnews&catid=19" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">Life Style Head line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="Intitle" placeholder="Life Style News Head line"
