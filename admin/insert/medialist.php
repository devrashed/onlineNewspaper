<?php if ($_GET['catid']=='newmedialist') { ?>
<div class='title'> <em>Media List News</em> </div>
...
<?php if ($_GET['catid']=='viewmedialist') { ?>
<div class='title'> <em>View All Media List News</em> </div>

<?php } if($_GET['catid']=='editmedia') {?>

<div class='title'> <em>Edit The Media List News</em> </div>
<?php } ?>
...
if($_POST['metitle']!='' && $_FILES['up_images']!='' && $_POST['reporter']!='')
 {
...
$query = "INSERT INTO newstype (`title`,`category`,`description`,`images`,`slideshow`,`author`,`nstatus`,`bracking`,`youtube`,`ctime`,`cdate`,`btime`,`bdate`) Values ('".$_POST['metitle']."','".media."','".$content."','".$filePath."','". $_POST['slide']."','".$_POST['reporter']."','". 1 ."','".$_POST['bknews'] ."'
...
if($Query2)
	...
	{
$msg= "Media List News Insert Successfully";
header("location:admin.php?action=medialist&catid=viewmedialist&msg=$msg"); //&msg=$msg
	
	}else{ 
	
$error= "Some Information Was Wrong";
header("location:admin.php?action=medialist&catid=newmedialist&error=$error"); //&error=$error
	  }
...
<form action="admin.php?action=medialist&catid=1" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">Media News Head line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="metitle" placeholder="Media News Head line"></div>
...
if($id!="")
{
    $query2= "select * FROM newstype WHERE category='media' AND nid='$id'";
    $rsquery2 =mysql_query($query2);
    $vquery2 = mysql_fetch_assoc($rsquery2);
    unlink($vquery2['images']);
	
	$qryDelete="DELETE FROM newstype WHERE category='media' AND nid='$id'";
	$rsDelete=mysql_query($qryDelete);
	
	if($rsDelete)
	{
		$demsg= "Delete Successfull";
		header("Location:admin.php?action=medialist&catid=viewmedialist&1&demsg=$demsg");
		die();
	}
	else {
	    $delerror= "Delete Failed";
		header("Location:admin.php?action=medialist&catid=viewmedialist&delerror=$delerror");
		die();
	}	
}
...
<td width="409" align="center">Media News Title</td>
...
<a href="admin.php?action=medialist&catid=editmedia&nid=<?php echo $vquery['nid'];?>"><i class="fa fa-edit" title="News Edit"></i></a>&nbsp;&nbsp;
<a href="admin.php?action=medialist&catid=viewmedialist&nid=<?php echo $vquery['nid'];?>" onclick="return confirmasubmit()"><i class="fa fa-trash-o" title="Delete News"></i></a>
...
<div class="paigination"><span class="paiglink"><?php pagination($page,$nrow,"admin.php?action=lifestyle&catid=viewlifestyle&", $perpa);?></span></div>
<div class="clr"></div>
</div>
<?php
  }
...
<?php
  $id= $_GET["nid"]; 
  $query= "SELECT * FROM newstype WHERE category='media' AND nid='".$_GET["nid"]."'";
  $rsquery= mysql_query($query); 
  $vquery= mysql_fetch_array($rsquery);
?>


<form action="admin.php?action=editnews&catid=15" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">Media News Head line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="metitle" placeholder="Media News Head line" 
