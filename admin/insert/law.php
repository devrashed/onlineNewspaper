<?php if ($_GET['catid']=='newlaw') { ?>
<div class='title'> <em>Law News</em> </div>
...
<?php if ($_GET['catid']=='viewlaw') { ?>
<div class='title'> <em>View All Law News</em> </div>



<?php } if($_GET['catid']=='editlaw') {?>

<div class='title'> <em>Edit The Law News</em> </div>
<?php } ?>
...
if($_POST['lawtitle']!='' && $_FILES['up_images']!='' && $_POST['reporter']!='' && $_POST['editor1']!='')
 {
...
$query = "INSERT INTO newstype (`title`,`category`,`description`,`images`,`slideshow`,`author`,`nstatus`,`bracking`,`youtube`,`ctime`,`cdate`,`btime`,`bdate`) Values ('".$_POST['lawtitle']."','".lawcourt."','".$content."','".$filePath."','". $_POST['slide']."','".$_POST['reporter']."','". 1 ."','".$_POST['bknews'] ."'
...
if($Query2)
	...
	{
$msg= "Law Insert Successfully";
header("location:admin.php?action=law&catid=viewlaw&msg=$msg"); //&msg=$msg
	
	}else{ 
	
$error= "Some Information Was Wrong";
header("location:admin.php?action=law&catid=newlaw&error=$error"); //&error=$error
	  }
...
<?php 
 if($_GET['catid']=='newlaw')
  {
?>

<div class="page">

<form action="admin.php?action=law&catid=1" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">law News Head line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="lawtitle" placeholder="Law News Head line"></div>
...
if($id!="")
{
    $query2= "select * FROM newstype WHERE category='lawcourt' AND nid='$id'";
    $rsquery2 =mysql_query($query2);
    $vquery2 = mysql_fetch_assoc($rsquery2);
    unlink($vquery2['images']);
	
	$qryDelete="DELETE FROM newstype WHERE category='lawcourt' AND nid = '$id'";
	$rsDelete=mysql_query($qryDelete);
	
	if($rsDelete)
	{
		$demsg= "Delete Successfull";
		header("Location:admin.php?action=law&catid=viewlaw&1&demsg=$demsg");
		die();
	}
	else {
	    $delerror= "Delete Failed";
		header("Location:admin.php?action=law&catid=viewlaw&delerror=$delerror");
		die();
	}	
}
...
<td width="409" align="center">Law Newe Title</td>
...
<a href="admin.php?action=law&catid=editlaw&nid=<?php echo $vquery['nid'];?>"><i class="fa fa-edit" title="News Edit"></i></a>&nbsp;&nbsp;
<a href="admin.php?action=law&catid=viewlaw&nid=<?php echo $vquery['nid'];?>" onclick="return confirmasubmit()"><i class="fa fa-trash-o" title="Delete News"></i></a>
</td>
...
<div class="paigination"><span class="paiglink"><?php pagination($page,$nrow,"admin.php?action=law&catid=viewlaw&", $perpa);?></span></div>
<div class="clr"></div>
</div>
...
<div class="page">
 ...
 <?php
  $id= $_GET["nid"]; 
  $query= "SELECT * FROM newstype WHERE category='lawcourt' AND nid='".$_GET["nid"]."'";
  $rsquery= mysql_query($query); 
  $vquery= mysql_fetch_array($rsquery);
?>

 <form action="admin.php?action=editnews&catid=8" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">law News Head line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="lawtitle" placeholder="Law News Head line" 
