<?php if ($_GET['catid']=='newhealth') { ?>
<div class='title'> <em>Health News</em> </div>
...
<?php if ($_GET['catid']=='viewhealth') { ?>
<div class='title'> <em>View All Health News</em> </div>



<?php } if($_GET['catid']=='edithealth') {?>

<div class='title'> <em>Edit The Health News</em> </div>
<?php } ?>
...
if($_POST['healtitle']!='' && $_FILES['up_images']!='' && $_POST['reporter']!='')
 {
...
$query = "INSERT INTO newstype (`title`,`category`,`description`,`images`,`slideshow`,`author`,`nstatus`,`bracking`,`youtube`,`ctime`,`cdate`,`btime`,`bdate`) Values ('".$_POST['healtitle']."','".health."','".$content."','".$filePath."','". $_POST['slide']."','".$_POST['reporter']."','". 1 ."','".$_POST['bknews'] ."'
...
if($Query2)
	...
	{
$msg= "Health News Insert Successfully";
header("location:admin.php?action=health&catid=viewhealth&msg=$msg"); //&msg=$msg
	
	}else{ 
	
$error= "Some Information Was Wrong";
header("location:admin.php?action=health&catid=newhealth&error=$error"); //&error=$error
	  }
...
<form action="admin.php?action=health&catid=1" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">Health News Head line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="healtitle" placeholder="Health News Head line"></div>
...
if($id!="")
{
    $query2= "select * FROM newstype WHERE category='health' AND nid='$id'";
    $rsquery2 =mysql_query($query2);
    $vquery2 = mysql_fetch_assoc($rsquery2);
    unlink($vquery2['images']);
	
	$qryDelete="DELETE FROM newstype WHERE category='health' AND nid='$id'";
	$rsDelete=mysql_query($qryDelete);
	
	if($rsDelete)
	{
		$demsg= "Delete Successfull";
		header("Location:admin.php?action=health&catid=viewhealth&1&demsg=$demsg");
		die();
	}
	else {
	    $delerror= "Delete Failed";
		header("Location:admin.php?action=health&catid=viewhealth&delerror=$delerror");
		die();
	}	
}
...
<td width="409" align="center">Health News Title</td>
...
<td width="56" align="center">
<a href="admin.php?action=health&catid=edithealth&nid=<?php echo $vquery['nid'];?>"><i class="fa fa-edit" title="News Edit"></i></a>&nbsp;&nbsp;
<a href="admin.php?action=health&catid=viewhealth&nid=<?php echo $vquery['nid'];?>" onclick="return confirmasubmit()"><i class="fa fa-trash-o" title="Delete News"></i></a>
</td>
...
<div class="paigination"><span class="paiglink"><?php pagination($page,$nrow,"admin.php?action=health&catid=viewhealth&", $perpa);?></span></div>
<div class="clr"></div>
</div>
<?php
  }
...
<?php
  $id= $_GET["nid"]; 
  $query= "SELECT * FROM newstype WHERE category='health' AND nid='".$_GET["nid"]."'";
  $rsquery= mysql_query($query); 
  $vquery= mysql_fetch_array($rsquery);
?>

 <form action="admin.php?action=editnews&catid=9" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">Health News Head line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="healtitle" placeholder="Health News Head line" 
