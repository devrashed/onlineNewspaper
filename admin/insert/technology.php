<?php if ($_GET['catid']=='newtechnology') { ?>
<div class='title'> <em>Technology News</em> </div>
...
<?php if ($_GET['catid']=='viewtechnology') { ?>
<div class='title'> <em>View All Technology News</em> </div>



<?php } if($_GET['catid']=='edittechnology') {?>

<div class='title'> <em>Edit The Technology News</em> </div>
<?php } ?>
...
if($_POST['techtitle']!='' && $_FILES['up_images']!='' && $_POST['reporter']!='')
 {
...
$query = "INSERT INTO newstype (`title`,`category`,`description`,`images`,`slideshow`,`author`,`nstatus`,`bracking`,`youtube`,`ctime`,`cdate`,`btime`,`bdate`) Values ('".$_POST['techtitle']."','".informationscience."','".$content."','".$_POST['slide']."','".$_POST['reporter']."','".$filePath."','". 1 ."','".$_POST['bknews'] ."','".$_POST['youtube']."','".$CUR_D_Bangladesh."','".$GM_D."','".$presenttime."','".$presentDate."')";
...
if($Query2)
	...
	{
$msg= "Information News Insert Successfully";
header("location:admin.php?action=technology&catid=viewtechnology&msg=$msg"); //&msg=$msg
	
	}else{ 
	
$error= "Some Information Was Wrong";
header("location:admin.php?action=technology&catid=newtechnology&error=$error"); //&error=$error
	  }
	  
	}
	else{
$error= "Some Information Was Wrong";	
header("location:admin.php?action=technology&catid=newtechnology"); //&error=$error
	}
   }
?>
...
<?php 
 if($_GET['catid']=='newtechnology')
  {
?>

<div class="page">

<form action="admin.php?action=technology&catid=1" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">Technology News Head line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="techtitle" placeholder="Information News Head line"></div>
...
if($id!="")
{
    $query2= "select * FROM newstype WHERE category='informationscience' AND nid='$id'";
    $rsquery2 =mysql_query($query2);
    $vquery2 = mysql_fetch_assoc($rsquery2);
    unlink($vquery2['images']);
	
	$qryDelete="DELETE FROM newstype WHERE category='informationscience' AND nid='$id'";
	$rsDelete=mysql_query($qryDelete);
	
	if($rsDelete)
	{
		$demsg= "Delete Successfull";
		header("Location:admin.php?action=technology&catid=viewtechnology&demsg=$demsg");
		die();
	}
	else {
	    $delerror= "Delete Failed";
		header("Location:admin.php?action=technology&catid=viewtechnology&delerror=$delerror");
		die();
	}	
}
...
<td width="409" align="center">Technology News Title</td>
...
<a href="admin.php?action=technology&catid=edittechnology&nid=<?php echo $vquery['nid'];?>"><i class="fa fa-edit" title="News Edit"></i></a>&nbsp;&nbsp;
<a href="admin.php?action=technology&catid=viewtechnology&nid=<?php echo $vquery['nid'];?>" onclick="return confirmasubmit()"><i class="fa fa-trash-o" title="Delete News"></i></a>
...
<div class="paigination"><span class="paiglink"><?php pagination($page,$nrow,"admin.php?action=technology&catid=viewtechnology&", $perpa);?></span></div> 
<div class="clr"></div>
</div>
<?php
  }
...
<?php
  $id= $_GET["nid"]; 
  $query= "SELECT * FROM newstype WHERE category='informationscience' AND nid='".$_GET["nid"]."'";
  $rsquery= mysql_query($query); 
  $vquery= mysql_fetch_array($rsquery);
?>

  
<form action="admin.php?action=editnews&catid=10" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">Technology News Head line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="techtitle" placeholder="Health News Head line" 
