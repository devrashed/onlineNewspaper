<?php if ($_GET['catid']=='newislam') { ?>
<div class='title'> <em>Islam News</em> </div>
...
<?php if ($_GET['catid']=='viewislam') { ?>
<div class='title'> <em>View All Islam News</em> </div>

<?php } if($_GET['catid']=='editnislam') {?>

<div class='title'> <em>Edit The Islam News</em> </div>
<?php } ?>
...
$query = "INSERT INTO newstype (`title`,`category`,`description`,`images`,`slideshow`,`author`,`nstatus`,`youtube`,`ctime`,`bracking`,`cdate`,`btime`,`bdate`) Values ('".$_POST['Istitle']."','".islam."','".$content."','".$filePath."','". $_POST['slide']."','".$_POST['reporter']."','". 1 ."','".$_POST['bknews'] ."',
...
header("location:admin.php?action=islam&catid=newislam&error=$error"); //&error=$error
	  }
...
<?php 
 if($_GET['catid']=='newislam')
  {
?>

<div class="page">

<form action="admin.php?action=islam&catid=1" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">Islam News Head line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="Istitle" placeholder="Islam News Head line"></div>
...
if($id!="")
{
    $query2= "select * FROM newstype WHERE category='islam' AND nid='$id'";
    $rsquery2 =mysql_query($query2);
    $vquery2 = mysql_fetch_assoc($rsquery2);
    unlink($vquery2['images']);
	
	$qryDelete="DELETE FROM newstype WHERE category='islam' AND nid='$id'";
	$rsDelete=mysql_query($qryDelete);
	
	if($rsDelete)
	{
		$demsg= "Delete Successfull";
		header("Location:admin.php?action=islam&catid=viewislam&demsg=$demsg");
		die();
	}
	else {
	    $delerror= "Delete Failed";
		header("Location:admin.php?action=islam&catid=viewislam&delerror=$delerror");
		die();
	}	
}
...
<td width="409" align="center">Islam News Title</td>
...
<a href="admin.php?action=islam&catid=editnislam&nid=<?php echo $vquery['nid'];?>"><i class="fa fa-edit" title="News Edit"></i></a>&nbsp;&nbsp;
<a href="admin.php?action=islam&catid=viewislam&nid=<?php echo $vquery['nid'];?>" onclick="return confirmasubmit()"><i class="fa fa-trash-o" title="Delete News"></i></a>
...
<div class="paigination"><span class="paiglink"><?php pagination($page,$nrow,"admin.php?action=islam&catid=viewislam&", $perpa);?></span></div>
<div class="clr">  </div>
</div>
...
<?php
 ...
 <div class="page">
  ...
  <?php
  $id= $_GET["nid"]; 
  $query= "SELECT * FROM newstype WHERE category='islam' AND nid='".$_GET["nid"]."'";
  $rsquery= mysql_query($query); 
 ...
 <form action="admin.php?action=editnews&catid=13" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">Islam News Head line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="Istitle" placeholder="Islam News Head line"
