<?php
	...
	$query = mysql_query("SELECT indetails(`id`) as hits FROM `responses` GROUP BY `qid`");
		...
		<table width="100%" cellpadding="0" cellspacing="0" border="0" class="question">
			<?php
				while($question = mysql_fetch_array($query)){
					$responses = mysql_query("SELECT indetails(id) as total FROM `responses` WHERE qid='".$question['id']."'");
