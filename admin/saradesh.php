...
if($_POST['saratitle']!='' && $_POST['division']!='0' && $_FILES['up_images']!='' && $_POST['reporter']!='')
 {
...
$query = "INSERT INTO newstype (`title`,`category`,`sardes_category`,`description`,`images`,`slideshow`,`author`,`nstatus`,`bracking`,`youtube`,`ctime`,`cdate`,`btime`,`bdate`) Values ('".$_POST['saratitle']."','".saradesh."','".$_POST['division']."','".$content."','".$filePath."','".$_POST['slide']."','".$_POST['reporter']."','". 1 ."','".$_POST['bknews'] ."','".$_POST['youtube']."','".$CUR_D_Bangladesh."','".$GM_D."','".$presenttime."','".$presentDate."')";
...
if($Query2)
	...
	{
$msg= "SaraDesh News Insert Successfully";
header("location:admin.php?action=saradesh&catid=viewsaradesh&msg=$msg"); //&msg=$msg
	
	}else{ 
	
$error= "Some Information Was Wrong";
header("location:admin.php?action=saradesh&catid=newsaradesh&error=$error"); //&error=$error
	  }
...
<div class="textbar"><label for="Newsheadline">Select The Division</label></div>
<div class="textbar"> 
...
if($id!="")
{
    $query2= "select * FROM newstyp nid='$id'";
    $rsquery2 =mysql_query($query2);
    $vquery2 = mysql_fetch_assoc($rsquery2);
    unlink($vquery2['images']);
	
	$qryDelete="DELETE FROM newstype WHERE category='saradesh' AND nid='$id'";
...
<td width="95" align="center">Division</td>
...
<td width="95" align="center" style="text-transform:capitalize;"> <?php echo $vquery['sardes_category'];?></td>
...
<td width="56" align="left"><a href="admin.php?action=saradesh&catid=editsaradesh&nid=<?php echo $vquery['nid'];?>"><i class="fa fa-edit" title="News Edit"></i></a>&nbsp;&nbsp;<a href="admin.php?action=saradesh&catid=viewsaradesh&nid=<?php echo $vquery['nid'];?>" onclick="return confirmasubmit()"><i class="fa fa-trash-o" title="Delete News"></i></a></td>
</tr>
<?php 
   }
?>
...
<div class="paigination"><span class="paiglink"><?php pagination($page,$nrow,"admin.php?action=saradesh&catid=viewsaradesh&", $perpa);?></span></div> 
</div>
...
<?php
  $id= $_GET["nid"]; 
  $query= "SELECT * FROM newstype WHERE category='saradesh' AND nid='".$_GET["nid"]."'";
  $rsquery= mysql_query($query); 
...
<form action="admin.php?action=editnews&catid=2" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">News Head line</label> </div>

<div class="textbar"> <input type="saratitle" class="form-control" id="saratitle" name="saratitle" placeholder="Sara Desh News Head Line" 
...
<option selected="selected" value="<?php echo $vquery['sardes_category'];?>"><?php echo $vquery['sardes_category'];?></option>
  
    <?php if ($vquery['sardes_category'] !='dhaka') {?>
	<option value="dhaka">Dhaka</option>
	<?php } if ($vquery['sardes_category'] !='jessore') {?>
	<option value="jessore">Jessore</option>
	<?php } if ($vquery['sardes_category'] !='chittagong') {?>
	<option value="chittagong">Chittagong</option>
    <?php } if ($vquery['sardes_category'] !='rajshahi') {?>
	<option value="rajshahi">Rajshahi</option>
    <?php } if ($vquery['sardes_category'] !='sylhet') {?>
	<option value="sylhet">Sylhet</option>
    <?php } if ($vquery['sardes_category'] !='khulna') {?>
	<option value="khulna">Khulna</option>
	<?php } ?>
