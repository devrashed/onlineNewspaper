<?php if ($_GET['catid']=='newpress') { ?>
<div class='title'> <em>Press News</em> </div>
...
<?php if ($_GET['catid']=='viewpress') { ?>
<div class='title'> <em>View All Press News </em> </div>

<?php } if($_GET['catid']=='editpress') {?>

<div class='title'> <em>Edit The Press News</em> </div>
<?php } ?>
...
$query = "INSERT INTO newstype (`title`,`category`,`ctime`,`cdate`,`btime`,`bdate`)
 Values ('".$_POST['optitle']."','".press."','".$CUR_D_Bangladesh."','".$GM_D."','".$presenttime."','".$presentDate."')";
...
if($Query2)
	...
	{
$msg= "Open Talk News Insert Successfully";
header("location:admin.php?action=pressclub&catid=viewpress&msg=$msg"); //&msg=$msg
	
	}else{ 
	
$error= "Some Information Was Wrong";
header("location:admin.php?action=pressclub&catid=newpress&error=$error"); //&error=$error
	  }
...
<form action="admin.php?action=pressclub&catid=1" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">Press News line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="optitle" placeholder="Press News line"></div>
...
if($id!="")
{
    $query2= "select * FROM newstype WHERE category='press' AND nid='$id'";
    $rsquery2 =mysql_query($query2);
    $vquery2 = mysql_fetch_assoc($rsquery2);
    unlink($vquery2['images']);
	
	$qryDelete="DELETE FROM newstype WHERE category='press' AND nid = '$id'";
	$rsDelete=mysql_query($qryDelete);
	
	if($rsDelete)
	{
		$demsg= "Delete Successfull";
		header("Location:admin.php?action=pressclub&catid=viewpress&1&demsg=$demsg");
		die();
	}
	else {
	    $delerror= "Delete Failed";
		header("Location:admin.php?action=pressclub&catid=viewpress&delerror=$delerror");
		die();
	}	
}
...
<td width="409" align="center">Open Talk News Title</td>
...
<?php
 $i=1;    
 $perpa=20;
 $re1=mysql_query("select count(nid) row FROM newstype WHERE category='press' ORDER BY nid DESC");
  ...
  //$i=1;
	...
	$query= "SELECT * FROM newstype WHERE category='press' order by nid DESC limit ".$start.",".$end;
...
<td width="56" align="center">
<a href="admin.php?action=pressclub&catid=editpress&nid=<?php echo $vquery['nid'];?>"><i class="fa fa-edit" title="News Edit"></i></a>&nbsp;&nbsp;
<a href="admin.php?action=pressclub&catid=viewpress&nid=<?php echo $vquery['nid'];?>" onclick="return confirmasubmit()"><i class="fa fa-trash-o" title="Delete News"></i></a>
</td>
</tr>
<?php 
   }
...
<div class="paigination"><span class="paiglink"><?php pagination($page,$nrow,"admin.php?action=opentalk&catid=viewpress&", $perpa);?></span></div>
<div class="clr">  </div>
</div>
...
<div class="page">
 ...
 <?php
  $id= @$_GET["nid"]; 
  $query= "SELECT * FROM newstype WHERE category='press' AND nid='".$_GET["nid"]."'";
  $rsquery= mysql_query($query); 
...
<div class="textbar"> <label for="Newsheadline">Press News</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="optitle" placeholder="Health News Head line"
