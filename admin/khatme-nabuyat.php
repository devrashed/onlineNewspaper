<?php if ($_GET['catid']=='newkhatme') { ?>
<div class='title'> <em>Khatme Nabuyat News</em> </div>
...
<?php if ($_GET['catid']=='viewkhatme') { ?>
<div class='title'> <em>View All Khatme Nabuyat</em> </div>

<?php } if($_GET['catid']=='editnkhatme') {?>

<div class='title'> <em>Edit The Khatme Nabuyat</em> </div>
<?php } ?>
...
if($_POST['Istitle']!='' && $_FILES['up_images']!='' && $_POST['reporter']!='')
 {
...
$query = "INSERT INTO newstype (`title`,`category`,`description`,`images`,`slideshow`,`author`,`nstatus`,`youtube`,`ctime`,`bracking`,`cdate`,`btime`,`bdate`) Values ('".$_POST['Istitle']."','".khatme."','".$content."','".$filePath."','". $_POST['slide']."','".$_POST['reporter']."','". 1 ."','".$_POST['bknews'] ."',
...
if($Query2)
	...
	{
$msg= "Islam News Insert Successfully";
header("location:admin.php?action=khatmenabuya&catid=viewkhatme&msg=$msg"); //&msg=$msg
	
	}else{ 
	
$error= "Some Information Was Wrong";
header("location:admin.php?action=khatmenabuya&catid=viewkhatme&error=$error"); //&error=$error
	  }
	  
	}else{
$error= "Some Information Was Wrong";	
header("location:admin.php?action=khatmenabuya&catid=newkhatme&error=$error"); //&error=$error
	}
   }
?>
...
<?php 
 if($_GET['catid']=='newkhatme')
  {
?>

<div class="page">

<form action="admin.php?action=khatmenabuya&catid=1" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">Khatme Nabuyat News Head line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="Istitle" placeholder="Khatme Nabuyat News Head line"></div>
...
if($id!="")
{
    $query2= "select * FROM newstype WHERE category='khatme' AND nid='$id'";
    $rsquery2 =mysql_query($query2);
    $vquery2 = mysql_fetch_assoc($rsquery2);
    unlink($vquery2['images']);
	
	$qryDelete="DELETE FROM newstype WHERE category='khatme' AND nid='$id'";
	$rsDelete=mysql_query($qryDelete);
	
	if($rsDelete)
	{
		$demsg= "Delete Successfull";
		header("Location:admin.php?action=khatmenabuya&catid=viewkhatme&demsg=$demsg");
		die();
	}
	else {
	    $delerror= "Delete Failed";
		header("Location:admin.php?action=khatmenabuya&catid=viewkhatme&delerror=$delerror");
		die();
	}	
}
...
<td width="409" align="center">Khatme Nabuyat News Title</td>
...
<a href="admin.php?action=khatmenabuya&catid=editnkhatme&nid=<?php echo $vquery['nid'];?>"><i class="fa fa-edit" title="News Edit"></i></a>&nbsp;&nbsp;
<a href="admin.php?action=khatmenabuya&catid=viewkhatme&nid=<?php echo $vquery['nid'];?>" onclick="return confirmasubmit()"><i class="fa fa-trash-o" title="Delete News"></i></a>
...
<div class="paigination"><span class="paiglink"><?php pagination($page,$nrow,"admin.php?action=khatmenabuya&catid=viewkhatme&", $perpa);?></span></div>
<div class="clr">  </div>
</div>
...
<?php
 ...
 <div class="page">
  ...
  <?php
  $id= $_GET["nid"]; 
  $query= "SELECT * FROM newstype WHERE category='khatme' AND nid='".$_GET["nid"]."'";
  $rsquery= mysql_query($query); 
 ...
 <form action="admin.php?action=editnews&catid=38" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">Khatme Nabuyat News Head line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="Istitle" placeholder="Khatme Nabuyat News Head line"
...
<div class="textbar"><input type="hidden" name="Regnid" value="<?php echo $vquery['nid']; ?>" /><button type="submit" class="btn btn-primary">Submit</button> </div>
