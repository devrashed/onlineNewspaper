<?php if ($_GET['catid']=='neweditorial') { ?>
<div class='title'> <em>Editorial  News</em> </div>
...
<?php if ($_GET['catid']=='vieweditorial') { ?>
<div class='title'> <em>View All Editorial  News</em> </div>

<?php } if($_GET['catid']=='editeditorial') {?>

<div class='title'> <em>Edit The Editorial News</em> </div>
<?php } ?>
...
if($_POST['eidtitle']!='' && $_FILES['up_images']!='' && $_POST['reporter']!='')
 {
...
$query = "INSERT INTO newstype (`title`,`category`,`description`,`images`,`slideshow`,`author`,`nstatus`,`bracking`,`youtube`,`ctime`,`cdate`,`btime`,`bdate`) Values ('".$_POST['eidtitle']."','".editorial."','".$content."','".$filePath."','".$_POST['slide']."','".$_POST['reporter']."','". 1 ."'
...
if($Query2)
	...
	{
$msg= "Editorail Insert Successfully";
header("location:admin.php?action=editorial&catid=vieweditorial&msg=$msg"); //&msg=$msg
	
	}else{ 
	
$error= "Some Information Was Wrong";
header("location:admin.php?action=editorial&catid=neweditorial&error=$error"); //&error=$error
	  }
...
<?php 
 if($_GET['catid']=='neweditorial')
  {
?>

<div class="page">

<form action="admin.php?action=editorial&catid=1" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">Editorial News Head line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="eidtitle" placeholder="Editorial News Head line"></div>
...
if($id!="")
{
    $query2= "select * FROM newstype WHERE category='editorial' AND nid='$id'"; 
    $rsquery2 =mysql_query($query2);
    $vquery2 = mysql_fetch_assoc($rsquery2);
    unlink($vquery2['images']);
	
	$qryDelete="DELETE FROM newstype WHERE category='editorial' AND nid = '$id'";
	$rsDelete=mysql_query($qryDelete);
	
	if($rsDelete)
	{
		$demsg= "Delete Successfull";
		header("Location:admin.php?action=editorial&catid=vieweditorial&demsg=$demsg");
		die();
	}
	else {
	    $delerror= "Delete Failed";
		header("Location:admin.php?action=editorial&catid=vieweditorial&delerror=$delerror");
		die();
	}	
}
...
<td width="409" align="center">Editorial News Title</td>
...
<a href="admin.php?action=editorial&catid=editeditorial&nid=<?php echo $vquery['nid'];?>"><i class="fa fa-edit" title="News Edit"></i></a>&nbsp;&nbsp;
<a href="admin.php?action=editorial&catid=vieweditorial&nid=<?php echo $vquery['nid'];?>" onclick="return confirmasubmit()"><i class="fa fa-trash-o" title="Delete News"></i></a>
...
<div class="paigination"><span class="paiglink"><?php pagination($page,$nrow,"admin.php?action=editorial&catid=vieweditorial&", $perpa);?></span></div>
<div class="clr"></div>
</div>
<?php
  }
...
<div class="page">
  ...
  <?php
  $id= $_GET["nid"]; 
  $query= "SELECT * FROM newstype WHERE category='editorial' AND nid='".$_GET["nid"]."'";
  $rsquery= mysql_query($query); 
...
<form action="admin.php?action=editnews&catid=11" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">Editorial News Head line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="eidtitle" placeholder="Editorial News Head line"
