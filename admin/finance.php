<?php if ($_GET['catid']=='newfinance') { ?>
<div class='title'> <em>Finance News</em> </div>
...
<?php if ($_GET['catid']=='viewfinance') { ?>
<div class='title'> <em>View All Finance News</em> </div>



<?php } if($_GET['catid']=='editfinance') {?>

<div class='title'> <em>Edit The Finance News</em> </div>
<?php } ?>
...
if($_POST['financtitle']!='' && $_FILES['up_images']!='' && $_POST['reporter']!='')
 {
...
$query = "INSERT INTO newstype (`title`,`category`,`description`,`images`,`slideshow`,`author`,`nstatus`,`bracking`,`youtube`,`ctime`,`cdate`,`btime`,`bdate`) Values ('".$_POST['financtitle']."','".finance."','".$content."','".$filePath."','". $_POST['slide']."','".$_POST['reporter']."','". 1 ."',
'".$_POST['bknews'] ."','".$_POST['youtube']."','".$CUR_D_Bangladesh."','".$GM_D."','".$presenttime."','".$presentDate."')";
...
if($Query2)
	...
	{
$msg= "Finance News Insert Successfully";
header("location:admin.php?action=finance&catid=viewfinance&msg=$msg"); //&msg=$msg
	
	}else{ 
	
$error= "Some Information Was Wrong";
header("location:admin.php?action=finance&catid=newfinance&error=$error"); //&error=$error
	  }
    }else{
$error= "Some Information Was Wrong";	
header("location:admin.php?action=finance&catid=newfinance&error=$error"); //&error=$error
...
<form action="admin.php?action=finance&catid=1" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">Finance News Head line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="financtitle" placeholder="International News Head line"></div>
...
if($id!="")
{
    $query2= "select * FROM newstype WHERE category='finance' AND nid='$id'";
    $rsquery2 =mysql_query($query2);
    $vquery2 = mysql_fetch_assoc($rsquery2);
    unlink($vquery2['images']);
	
	$qryDelete="DELETE FROM newstype WHERE category='finance' AND nid = '$id'";
	$rsDelete=mysql_query($qryDelete);
	
	if($rsDelete)
	{
		$demsg= "Delete Successfull";
		header("Location:admin.php?action=finance&catid=viewfinance&1&demsg=$demsg");
		die();
	}
	else {
	    $delerror= "Delete Failed";
		header("Location:admin.php?action=finance&catid=viewfinance&delerror=$delerror");
		die();
	}	
}
...
<td width="409" align="center">Finance  Newe Title</td>
...
<a href="admin.php?action=finance&catid=editfinance&nid=<?php echo $vquery['nid'];?>"><i class="fa fa-edit" title="News Edit"></i></a>&nbsp;&nbsp;
<a href="admin.php?action=finance&catid=viewfinance&nid=<?php echo $vquery['nid'];?>" onclick="return confirmasubmit()"><i class="fa fa-trash-o" title="Delete News"></i></a>
</td>
</tr>
<?php 
   }
...
<div class="page">
 ...
 <?php
  $id= $_GET["nid"]; 
  $query= "SELECT * FROM newstype WHERE category='finance' AND nid='".$_GET["nid"]."'";
  $rsquery= mysql_query($query); 
  $vquery= mysql_fetch_array($rsquery);
?>

<form action="admin.php?action=editnews&catid=5" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">Finance News Head line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="financtitle" placeholder="Finance News Head line" 
...
<div class="textbar"><textarea id="editor1" name="editor1" class="form-control"><?php echo $vquery['description'];?>
 ...
 <div class="textbar">Bracking : &nbsp;&nbsp;<input name="bknews" type="checkbox" checked="checked" value="yes" /></div> 
 <?php } elseif($vquery['bracking']!='yes') { ?>
 <div class="textbar">Bracking : &nbsp;&nbsp;<input name="bknews" type="checkbox"  value="yes" /></div> 
 <?php } ?>
