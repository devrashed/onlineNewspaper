...
<?php if ($_GET['catid']=='viewconsult') { ?>
<div class='title'> <em>View All Consultant News</em> </div>

<?php } if($_GET['catid']=='editconsult') {?>
...
if (agree)
    ...
    <?php
        $query= "SELECT * FROM self WHERE category='consultant' order by sfid DESC";
        $rsquery= mysql_query($query); 
        while ($vquery=mysql_fetch_array($rsquery))
       {
     ?>
    <tr> 
    <td width="409" align="center" style="font-family:SolaimanLipi">Consultant </td>
    </td>
    <td width="56" align="left">
	<a href="admin.php?action=consultant&catid=editconsult&sfid=<?php echo $vquery['sfid'];?>"><i class="fa fa-edit" title="News Edit"></i></a>&nbsp;&nbsp;
    </td>
    </tr>
    <?php 
       }
...
<div class="page"> 
 ...
 <?php
  $id= $_GET["sfid"]; 
  $query= "SELECT * FROM self WHERE category='consultant' AND sfid='".$_GET["sfid"]."'";
  $rsquery= mysql_query($query); 
  $vquery= mysql_fetch_array($rsquery);
?>


<form action="admin.php?action=editnews&catid=29" method="post" enctype="multipart/form-data">
