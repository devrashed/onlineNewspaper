...
mouseover: true, //when clicking outside of any "toggle" method menu, should all "toggle" menus close?  
...
executelink: function($, prevobjs, e){
	var prevsindetails = prevobjs.length, link = e.target;
	while(--prevsindetails > -1){
		if(prevobjs[prevsindetails] === this){
			prevobjs.splice(prevsindetails, 1);
...
updateprev: function($, prevobjs, $curobj){
	var prevsindetails = prevobjs.length, prevobj, $indexobj = $curobj.parents().add(this);
	while(--prevsindetails > -1){
		if($indexobj.index((prevobj = prevobjs[prevsindetails])) < 0){
...
closeall: function(e){
	var smoothmenu = ddsmoothmenu, prevsindetails;
	if(!smoothmenu.globaltrackopen){return;}
	if(e.type === 'mouseleave' || ((e.type === 'click' || e.type === 'touchstart') && smoothmenu.menus.index(e.target) < 0)){
		prevsindetails = smoothmenu.prevobjs.length;
		while(--prevsindetails > -1){
			$(smoothmenu.prevobjs[prevsindetails]).trigger('click');
			smoothmenu.prevobjs.splice(prevsindetails, 1);
		}
	}
},
