<?php if ($_GET['catid']=='newdor') { ?>
<div class='title'> <em>Dorbar Sharif News</em> </div>
...
<?php if ($_GET['catid']=='viewdor') { ?>
<div class='title'> <em>View All Dorbar Sharif News</em> </div>

<?php } if($_GET['catid']=='editndor') {?>

<div class='title'> <em>Edit The Dorbar Sharif News</em> </div>
<?php } ?>
...
$query = "INSERT INTO newstype (`title`,`category`,`description`,`images`,`slideshow`,`author`,`nstatus`,`youtube`,`ctime`,`bracking`,`cdate`,`btime`,`bdate`) Values ('".$_POST['Istitle']."','".dorbarsharif."','".$content."','".$filePath."','". $_POST['slide']."','".$_POST['reporter']."','". 1 ."','".$_POST['bknews'] ."',
...
header("location:admin.php?action=dorbarsharif&catid=viewdor&msg=$msg"); //&msg=$msg
	
	}else{ 
	
$error= "Some Information Was Wrong";
header("location:admin.php?action=dorbarsharif&catid=newdor&error=$error"); //&error=$error
	  }
    }
	else{
$error= "Some Information Was Wrong";	
...
<?php 
 if($_GET['catid']=='newdor')
  {
?>

<div class="page">

<form action="admin.php?action=dorbarsharif&catid=1" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">Dorbar Sharif News Head line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="Istitle" placeholder="Dorbar Sharif Head line"></div>
...
if($id!="")
{
    $query2= "select * FROM newstype WHERE category='dorbarsharif' AND nid='$id'";
    $rsquery2 =mysql_query($query2);
    $vquery2 = mysql_fetch_assoc($rsquery2);
    unlink($vquery2['images']);
	
	$qryDelete="DELETE FROM newstype WHERE category='dorbarsharif' AND nid='$id'";
	$rsDelete=mysql_query($qryDelete);
	
	if($rsDelete)
	{
		$demsg= "Delete Successfull";
		header("Location:admin.php?action=dorbarsharif&catid=viewdor&demsg=$demsg");
		die();
	}
	else {
	    $delerror= "Delete Failed";
		header("Location:admin.php?action=dorbarsharif&catid=viewdor&delerror=$delerror");
		die();
	}	
}
...
<td width="409" align="center">Dorbar Sharif News Title</td>
...
<a href="admin.php?action=dorbarsharif&catid=editndor&nid=<?php echo $vquery['nid'];?>"><i class="fa fa-edit" title="News Edit"></i></a>&nbsp;&nbsp;
<a href="admin.php?action=dorbarsharif&catid=viewdor&nid=<?php echo $vquery['nid'];?>" onclick="return confirmasubmit()"><i class="fa fa-trash-o" title="Delete News"></i></a>
...
<div class="paigination"><span class="paiglink"><?php pagination($page,$nrow,"admin.php?action=dorbarsharif&catid=viewdor&", $perpa);?></span></div>
<div class="clr">  </div>
</div>
...
<?php
 ...
 <div class="page">
  ...
  <?php
  $id= $_GET["nid"]; 
  $query= "SELECT * FROM newstype WHERE category='dorbarsharif' AND nid='".$_GET["nid"]."'";
  $rsquery= mysql_query($query); 
 ...
 <form action="admin.php?action=editnews&catid=37" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">Dorbar Sharif Head line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="Istitle" placeholder="Dorbar Sharif News Head line"
