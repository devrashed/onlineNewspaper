...
<?php ?>
<?php if ($_GET['catid']=='newpolitics') { ?>
<div class='title'> <em>Politics News</em> </div>
...
<?php if ($_GET['catid']=='viewpolitics') { ?>
<div class='title'> <em>View All Politics News</em> </div>

<?php } if($_GET['catid']=='editpolitics') {?>

<div class='title'> <em>Edit The Politics News</em> </div>
<?php } ?>


<?php 

//var_dump($_SESSION); //exit(aaa);
...
if($_POST['pcstitle']!='' && $_POST['reporter']!='' && $_FILES['up_images']!='')
 {
...
$query = "INSERT INTO newstype (`title`,`category`,`description`,`images`,`slideshow`,`author`,`nstatus`,`bracking`,`youtube`,`important`,`ctime`,`cdate`,`btime`,`bdate`) Values ('".$_POST['pcstitle']."','".politics."','".$content."','".$filePath."','".$_POST['slide']."','".$_POST['reporter']."','". 1 ."','".$_POST['bknews'] ."',
...
if($Query2)
	...
	{
$msg= "Politics News Insert Successfully";
header("location:admin.php?action=politics&catid=viewpolitics&msg=$msg");
	
	}else{ 
	
$errors= "Some Information Was Wrong";
header("location:admin.php?action=politics&catid=newpolitics&error=$error"); 
	  }
...
<form action="admin.php?action=politics&initem=nation_in" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">News Head line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="pcstitle" placeholder="Politics News Head Line"></div>
...
if($id!="")
{
    $query2= "select * FROM newstype WHERE category='politics' AND nid='$id'";
    $rsquery2 =mysql_query($query2);
    $vquery2 = mysql_fetch_assoc($rsquery2);
    unlink($vquery2['images']);
	
	$qryDelete="DELETE FROM newstype WHERE category='politics' AND nid='$id'";
	$rsDelete=mysql_query($qryDelete);
	
	if($rsDelete)
	{
		$demsg= "Delete Successfull";
		header("Location:admin.php?action=politics&catid=viewpolitics&1&demsg=$demsg");
		die();
	} else {
	    $delerror= "Delete Failed";
		header("Location:admin.php?action=politics&catid=viewpolitics&delerror=$delerror");
		die();
	}	
}
...
<a href="admin.php?action=politics&catid=editpolitics&nid=<?php echo $vquery['nid'];?>"><i class="fa fa-edit" title="News Edit"></i></a>&nbsp;&nbsp;
<a href="admin.php?action=politics&catid=viewpolitics&nid=<?php echo $vquery['nid'];?>" onclick="return confirmasubmit()"><i class="fa fa-trash-o" title="Delete News"></i></a>
...
<div class="paigination"><span class="paiglink"><?php pagination($page,$nrow,"admin.php?action=politics&catid=viewpolitics&", $perpa);?></span></div>
</div>  
...
<?php
  $id= $_GET["nid"]; 
  $query= "SELECT * FROM newstype WHERE category='politics' AND nid='".$_GET["nid"]."'";
  $rsquery= mysql_query($query); 
...
<form action="admin.php?action=editnews&catid=21" method="post" enctype="multipart/form-data">

<div class="textbar"> <label for="Newsheadline">News Head line</label> </div>

<div class="textbar"> <input type="newstitle" class="form-control" id="newstitle" name="ptitle" placeholder="News Head Line" 
...
<div class="textbar"><textarea id="editor1" name="editor1" class="form-control"> <?php echo $vquery['description'];?>  </textarea>
 ...
 <div class="textbar">Slideshow : &nbsp;&nbsp;<input name="bknews" type="checkbox" checked="checked" value="yes" /></div> 
 <?php } elseif($vquery['bracking']!='yes') { ?>
 <div class="textbar">Slideshow : &nbsp;&nbsp;<input name="bknews" type="checkbox"  value="yes" /></div> 
 <?php } ?>
