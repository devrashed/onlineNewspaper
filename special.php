 <?php
 $query= "SELECT * FROM newstype WHERE category='specialfeature' order by nid DESC LIMIT 3";
...
<?php $content1=$vquery2['description']; $content=preg_replace("/<\/?div[^>]*\>/i", "", $content1);  echo $content2=strip_tags(substr($content, 0, 300)); ?>
...
<?php
	...
	/*$query= "SELECT * FROM newstype WHERE category='specialfeature' order by nid DESC LIMIT 1, 6";
	$rsquery= mysql_query($query); 
	while ($vquery2=mysql_fetch_array($rsquery))
    {*/
?>
...
<div class="po-de"><a href="inn.php?catid=specialfeature">আরো খবর</a></div>
